...
export async function assertCanEditCrm(actorId: string, role: "ADMIN" | "EMPLOYEE") {
  if (role === "ADMIN") return;
  const perms = await getPermissions(actorId, role);
...
export async function createLeadCore(
  ...
  await prisma.leadActivity.create({
    data: { leadId: lead.id, userId: actor.id, message: `Лид создан` },
  });
...
export async function moveLeadCore(actor: Actor, leadId: string, toStage: LeadStageId, toIndex: number) {
  await assertCanEditCrm(actor.id, actor.role);
  ...
  await prisma.$transaction(
    siblings.map((s, idx) => prisma.lead.update({ where: { id: s.id }, data: { order: idx, stage: toStage } }))
  );

  if (lead.stage !== toStage) {
    await prisma.leadActivity.create({
      data: { leadId, userId: actor.id, message: `Стадия изменена: ${lead.stage} → ${toStage}` },
    });
    await syncLeadIncomeCore(actor, leadId);
  }
}
...
export async function updateLeadCore(
  ...
  await prisma.leadActivity.create({
    data: { leadId, userId: actor.id, message: `Карточка обновлена` },
  });

  await syncLeadIncomeCore(actor, leadId);
}

export async function setLeadLostCore(actor: Actor, leadId: string, lost: boolean, lostReason?: string) {
  await assertCanEditCrm(actor.id, actor.role);
...
async function syncLeadIncomeCore(actor: Actor, leadId: string) {
  const lead = await prisma.lead.findUniqueOrThrow({ where: { id: leadId } });
  await applyIncomeRules(lead, actor.id);
}
...
export async function reconcileAllLeadIncomeCore(actor: Actor) {
